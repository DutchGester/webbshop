...
// alert(2);
$(".containerDropdownTogglerMenu").css('padding-top', '30px');
$(".mobiletitleheader").css('padding-top', '14px');
 $(".hideCartForDesktop").css("pointer-events", "none");
  }
});
...
setTimeout(function() {
            $(".shopping-cart-header,#cart-count,.navbar-toggler,.mobiletitleheader").fadeOut(300);
        }, 0);
        ...
        setTimeout(function() {
              ...
              $('.searchInputHeader').fadeIn(300);
              $('.crossInput').fadeIn(300);
    ...
    if (doesWordExists) {
     ...
     $.ajax({
       ...
       success: function(response) {
         $('#added-to-cart-cart-count').text(response);
...
//remove product from cart
			 ...
			 $.ajax({
				 ...
				 data: {
				 	...
				 	dataType : 'JSON',
				 success: function(response) {
					  var cart_count = $('#cart-count').text(response['cart_total']);
						...
						if(response['cart_total'] == 0) {
							...
							$('.whole-product-div[data-id="'+id+'"]').fadeOut(550, function(){ $(this).remove();});
						}
