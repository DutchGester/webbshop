...
class Pagination{
	...
	var $fullTagOpen	= '<div class="pagination" style="justify-content: center;margin-bottom:50px;">';
	var $fullTagClose	= '</div>';
	var $firstTagOpen	= '';
	var $firstTagClose	= '';
	var $lastTagOpen	= '';
	var $lastTagClose	= '';
		var $curTagOpen		= '<li class="page-item active"><a class="page-link" onclick="return false;">';
	...
	function createLinks(){
		...
		if ($numPages == 1){
			if ($this->showCount){
				$info = '<p class="showingResultsInformation">Showing : <b>' . $this->totalRows.'</b></p>';
				return $info;
			}else{
				return '';
			}
		...
		if ($this->showCount){
		   ...
		   $info = '<p class="showingResultsInformation">Showing ' . ( $currentOffset + 1 ) . ' to ' ;
